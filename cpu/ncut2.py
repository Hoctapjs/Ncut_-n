...
def construct_graph(image, sigma_color, sigma_spatial):
    ...
    # Tính khoảng cách không gian và màu
    distances_spatial = euclidean_distances(coords, coords)
    distances_color = euclidean_distances(colors, colors)

    # Tính trọng số
    W = np.exp(-distances_color**2 / (2 * sigma_color**2)) * np.exp(-distances_spatial**2 / (2 * sigma_spatial**2))
...
def compute_eigenvectors(L, k=2):
    ...
    eigenvalues, eigenvectors = eigsh(L, k=k, which='SM', tol=1e-5, maxiter=20000)
    return eigenvectors
...
if __name__ == "__main__":
    ...
    scale = 0.5  # Tỷ lệ giảm kích thước
